...
class SchemaBuilder {
  ...
  _relationFields(modelData) {
    ...
    _.forOwn(modelData.modelClass.getRelations(), (relation) => {
      // console.log('relation.relatedModelClass ', relation.relatedModelClass.tableName)
      let tableName = relation.relatedModelClass.tableName;
      if(tableName.includes('.')) {
        tableName = relation.relatedModelClass.tableName.split('.')[1]
      }
      const relationModel = this.models[tableName];
  ...
  _filterForArgs(astNode, modelClass, variables) {
    ...
    if(tableName.includes('.')) {
      tableName = modelClass.tableName.split('.')[1]
    }

    const modelData = this.models[tableName];
